...
using Game.Scripts.OtherScripts.Doors;
...
namespace Game.Scripts.InventoryScripts
{
    public class QuickSlotInventory : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < quickSlotParent.childCount; i++)
            {
                if (Input.GetKeyDown((i + 1).ToString()))
                {
                    ...
                    {
                        if (quickSlotParent.GetChild(currentQuickSlotID).GetComponent<Image>().sprite ==
                            notSelectedSprite)
                        {
                            quickSlotParent.GetChild(currentQuickSlotID).GetComponent<Image>().sprite = selectedSprite;
                            activeSlot = quickSlotParent.GetChild(currentQuickSlotID).GetComponent<InventorySlot>();
                        ...
                        {
                            quickSlotParent.GetChild(currentQuickSlotID).GetComponent<Image>().sprite =
                                notSelectedSprite;
                            activeSlot = null;
                            HideItemsInHand();
                        }
        ...
        {
            indicators.ChangeHealthAmount(quickSlotParent.GetChild(currentQuickSlotID).GetComponent<InventorySlot>()
                .item.changeHealth);
        }
